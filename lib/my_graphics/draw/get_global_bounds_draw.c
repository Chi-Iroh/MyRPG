    ...
    switch (draw->type) {
        case SHAPE:
            ...
            if (shape->type == CIRCLE) {
                rect = sfCircleShape_getLocalBounds((sfCircleShape *)(shape->shape));
            } if (shape->type == RECT) {
        ...
        break;
        case SPRITE:
            rect = sfSprite_getLocalBounds(((sprite_t *)(draw->drawable))->sprite);
        break;
        case TEXT:
            rect = sfText_getLocalBounds(((text_t *)(draw->drawable))->text);
        break;
        default: break;
    }
    return rect;
