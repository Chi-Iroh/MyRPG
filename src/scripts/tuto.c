...
void tuto(window_t* wd, game_src_t* g_src)
{
    ...
    pos = g_src->game->crowd->player->draw->data->position; switch (step) {
            case 0: my_function0(g_src, clock, &step, ami); break;
            case 1: my_function1(g_src, clock, &step, ami); break;
            case 2: my_function2(g_src, clock, &step, ami); break;
            case 3: my_function3(g_src, clock, &step, ami); break;
            case 4: my_function4(g_src, clock, &step, ami); break;
            case 5: my_function5(g_src, &move, &step, var); break;
            case 6: my_function6(g_src, clock, &step, ami); break;
            case 7: my_function7(g_src, clock, &step, ami); break;
            case 8: if (time > 1.5) {speech_bubble_clean(g_src->game); step++;
            } break; default: break; g_src->tuto = false;
